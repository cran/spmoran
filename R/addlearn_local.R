addlearn_local<-function( mod, meig0 = NULL, x0 = NULL, xconst0=NULL, xgroup0=NULL,
  ...
  {
  if( mod$other$y_nonneg ) stop( "Error: This function is yet not supported for non-Gaussian data" )
  y       <- mod$other$y
  x       <- mod$other$x
  ...
  if( inherits(mod, "besf_vc") ){
    if( !is.null(meig0) ) message("Note: besf_vc is not yet supported for spatial prediction. Use resf_vc" )
  ...
  resf_vc_internal<- function( y, x, xconst = NULL, xgroup = NULL, weight = NULL, offset = NULL,
    ...
    null_dum3[mod_ref$other$idd %in% which(mod_sub2$vc==0)]<-FALSE
    ...
    #null_dum3 <- rep(TRUE,length(mod_ref$vc))
    #for(i in 1:length(mod_ref$other$evSqrts)){
    #  ev_sub  <- mod_ref$other$evSqrts[[i]]
    #  if( identical( ev_sub, 0 )==FALSE ){
    #    null_dum3<-c(null_dum3, rep(TRUE, length(meig$ev)) )
    #  } else {
    #    null_dum3<-c(null_dum3, rep(FALSE, length(meig$ev)) )
    #  }
    #}
    ...
    other		<- list(  r = r, sf_alpha = parR, x_id = x_id, nx=nx, nxf = nxf, xf_id = xf_id, df = df,null_dum3=null_dum3,
  ...
  #for(k in 1:cl_num){
                                   ...
                                   if( sum(id_sel1)!=0 | sum(id_sel2)!=0 ){
                                     ...
                                     nev0       <-length(mod_sub2$other$evSqrts[[1]])
                                     meig_sub$sf<-meig_sub$sf[,1:nev0]
                                     meig_sub$ev<-meig_sub$ev[ 1:nev0]
                                   ...
                                   if( !is.null( xconst_sub2 ) ){
                                     if( ncol( xconst_sub2 )!= nrow( Mod0$c ) ){# reshape b_vc and bse_vc in Mod0
                                       ...
                                       names(c_rev)   <- c("Estimate","SE","z_value","p_value")
                                       Mod0$c         <- c_rev
                                     }
                                   }
  ...
  names(Beta)<- names(Beta_se)<-names(Beta_z)<-names( Mod[[1]][1][[1]]$b_vc )
  ...
  rownames(sm_global)<-c("random_SD(global)","Moran.I/max(Moran.I)(global)")
  ...
  other   <- list(C=C,C_se,R=R,Wei=Wei,Wei0 = Wei0, Beta0=Beta0,Beta0_se=Beta0_se,
                  ...
                  Mod_local=Mod_local,Mod_global=Mod_global,x=x)

  result  <- list(b_vc=Beta,bse_vc=Beta_se, z_vc=Beta_z, p_vc=Beta_p,c=cest,b_g=b_g,
